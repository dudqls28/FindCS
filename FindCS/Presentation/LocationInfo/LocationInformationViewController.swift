...
extension Reactive where Base : LocationInformationViewController {
    ...
    var addPOIIitems : Binder<[MTMapPoint]>{
        return Binder(base) { base, points in
            let items = points
                .enumerated()
                .map { offset, point -> MTMapPOIItem in
                    let mapPOIIitem = MTMapPOIItem()
                    
                    mapPOIIitem.mapPoint = point
                    mapPOIIitem.markerType = .redPin
                    mapPOIIitem.showAnimationType = .springFromGround
                    mapPOIIitem.tag = offset
